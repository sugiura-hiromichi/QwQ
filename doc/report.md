...
このような情報から魔法数は導かれる。
具体的には2,8,20,28,50,82,126が魔法数と呼ばれている。
...
これを近似して、調和振動子ポテンシャルとして扱う。
つまり
$$V(r)=-V_0+\frac{m\omega^2r^2}{2}$$
ここで
$-V_0$は定数、mは核子の質量、$\omega$は調和振動子の角速度である。

核子がこのポテンシャル内で運動しているとすると、そのシュレディンガー方程式は
$$(-\frac{\hbar^2}{2m}\nabla^2-V_0+\frac{m\omega^2r^2}{2})\phi(\bm{r})=E\phi(\bm{r})$$
となる。
$r^2=x^2+y^2+z^2$なので、この方程式は、各方向に分けることができる。
変数分離できると仮定して、

具体的には、
$$(-\frac{\hbar^2}{2m}\frac{\partial^2}{\partial x^2}-V_{0_x}+\frac{m\omega^2x^2}{2})\phi(x)=E_x\phi(x)$$
$$(-\frac{\hbar^2}{2m}\frac{\partial^2}{\partial y^2}-V_{0_y}+\frac{m\omega^2y^2}{2})\phi(y)=E_y\phi(y)$$
$$(-\frac{\hbar^2}{2m}\frac{\partial^2}{\partial z^2}-V_{0_z}+\frac{m\omega^2z^2}{2})\phi(z)=E_z\phi(z)$$
また、
$$E=E_x+E_y+E_z$$
...
$$\phi(\bm{r})=\phi_{n_x}(x)\phi_{n_y}(y)\phi_{n_z}(z)$$
...
$$j(j+1)=l(l+1)+\frac{1}{2}\times\frac{3}{2}+2\bm{l}\cdot\bm{s}$$
$$\bm{l}\cdot \bm{s}=\frac{1}{2}\{j(j+1)-l(l+1)-\frac{3}{4}\}$$
つまり、
$$\bm{l}\cdot \bm{s}=
\begin{cases}
...
原子核の磁気モーメントの単位の核磁子は$\mu_N=e\hbar/(2Mc)=5.05\times 10^{-27}J/T^{-1}$と、かなり小さい。
...
と書く。
$\bm{\mu}=(g_s-g_l)\bm{s}+g_l\bm{j}$を用いると
...
$^{57}Fe$[^e]
と$^{111}Cd$[^f]の磁気モーメントの計測値は以下の通り。
$$^{57}Fe \quad 0.0906\mu_N$$
$$^{111}Cd \quad -0.594\mu_N$$

[^e]: https://www-nds.iaea.org/nuclearmoments/isotope_measurement_results.php?A=111&Z=48
[^f]: https://www-nds.iaea.org/nuclearmoments/isotope_measurement_results.php?A=57&Z=26

まず$^{57}Fe$の場合、この時最外殻のスピンは1/2。
また$^{57}Fe$は中性子数が奇数である。
よって
$$\mu_s=\frac{1}{2}\{\pm\frac{-3.826}{2(1/2\pm 1/2)+1}\}$$
$$=\pm 0.638, \pm 1.913$$
...
実際貴ガス原子核の磁気双極子モーメントでも、
$$^3He \quad -2.128\mu_N$$
$$^{20}Ne \quad 1.08\mu_N$$
などは大きい値を示している。
